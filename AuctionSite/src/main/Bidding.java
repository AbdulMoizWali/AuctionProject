...
public class Bidding {
	...
	public String getAuctionProduct() {
		  try {
		    ...
		    Statement stmt = con.createStatement();
		    String view = "SELECT `auctioned product`.`AuctionID`,product.Title,product.Category,product.Description,product.Location,`auctioned product`.Ended,`auctioned product`.Status,user.First_Name,`user buy auction`.BidAmount from product  left JOIN `auctioned product` on product.ProductID =`auctioned product`.ProductID left join `user buy auction` on `auctioned product`.`AuctionID` =`user buy auction`.`AuctionID`  left join user on `user buy auction`.`UserID`=user.UserID ";
		    ResultSet rs = stmt.executeQuery(view);
		    int i = 0;
		    rs.last();
